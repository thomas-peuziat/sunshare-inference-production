...
if __name__ == '__main__':
    ...
    # Caracteristiques installation en kW
    MAX_POWER_WIND = 2
    MAX_POWER_SOLAR = 10

    # Generation des fichiers .joblib
    TRAINING = True

    dataset = data.init_dataset(pv_dataset_path=pv_dataset_path, wind_dataset_path=wind_dataset_path)

    if TRAINING:
        # data.fit_linear_regression(dataset=dataset)
        data.fit_gradient_boosting_regression(dataset=dataset)
    ...
    pprint(daily_data)
    print("----------")
    print("Puissance installation éolienne :", MAX_POWER_WIND, "kw")
    print("Puissance installation solaire :", MAX_POWER_SOLAR, "kw")
    print("----------")
    print("Prédictions production :")
    pprint(predictions)
    print("----------")
    print("Temps de réponse total:", end-start, "s")
